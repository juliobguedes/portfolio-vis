...
const setUp = (data) => data.map(element => {
    element.dataEmissao = timeParser(element.dataEmissao);
    return element;
}).sort((a,b) => {
...
const ready = (error, dados) => {
    ...
    const finalDados = setUp(dados);
    
    x.domain(d3.extent(finalDados, (d, i) => d.dataEmissao));
    y.domain(d3.extent(finalDados, (d, i) => d.valorTotal));
